...
def _is_private_or_local_ip(ip_address):
    """Проверяет, что IP локальный/служебный и не требует внешнего geo lookup."""
    try:
...
def get_ip_region_label(ip_address):
    """Возвращает читаемую гео-метку по IP с кэшированием в БД."""
    if not ip_address:
        return "Неизвестно"

    if _is_private_or_local_ip(ip_address):
        return "Локальный/служебный IP"

    conn = None
    try:
        conn = sqlite3.connect(DATABASE_NAME)
        cursor = conn.cursor()
        cursor.execute('SELECT label FROM ip_geolocation WHERE ip_address = ?', (ip_address,))
        row = cursor.fetchone()
        if row and row[0]:
            return row[0]
        ...
        if payload.get('status') == 'success':
            ...
            label = ", ".join([part for part in (country, region, city) if part])
        else:
            country, region, city = '', '', ''
            label = 'Регион не определён'

        cursor.execute('''
            INSERT OR REPLACE INTO ip_geolocation
            (ip_address, country, region, city, label, updated_at)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (ip_address, country, region, city, label, datetime.now().isoformat()))
        conn.commit()
        return label
    except Exception as e:
        logger.warning(f"Не удалось определить регион для IP {ip_address}: {e}")
    ...
    try:
        ...
        for row in cursor.fetchall():
            applications.append({
                ...
                'message': row[6],
                'ip_address': row[7],
                'region': get_ip_region_label(row[7]),
                'timestamp': row[8],
...
def get_custom_gallery_images_for_admin():
    """Список добавленных (не стартовых) фото для управления в админке."""
    try:
        conn = sqlite3.connect(DATABASE_NAME)
        cursor = conn.cursor()
        cursor.execute('''
            SELECT id, image_url, added_by, added_at
            FROM gallery_images_custom
            WHERE is_active = 1
            ORDER BY id DESC
        ''')
        rows = cursor.fetchall()
        ...
        logger.error(f"Ошибка получения пользовательских фото: {e}")
        return []


def delete_custom_gallery_image(image_id):
    """Удаляет пользовательское (не стартовое) фото из галереи."""
    try:
        conn = sqlite3.connect(DATABASE_NAME)
        cursor = conn.cursor()
        cursor.execute('''
        ...
        logger.error(f"Ошибка удаления фото из галереи: {e}")
        return False, "Внутренняя ошибка"
...
class ClanRequestHandler(BaseHTTPRequestHandler):
    ...
    def handle_admin_request(self, path):
        ...
        elif path == '/admin/api/gallery/custom':
            self.serve_custom_gallery_images()
    ...
    def handle_admin_post_request(self, path):
        ...
        elif path == '/admin/api/gallery/delete':
            self.handle_delete_gallery_image()
        else:
            self.send_error(404)
    ...
    def serve_custom_gallery_images(self):
        """Список пользовательских фото для удаления (только главный админ)."""
        session = get_current_admin(self.headers.get('Cookie', ''))
        if not session or not session.get('is_super_admin'):
            self.send_error(403)
            return

        items = get_custom_gallery_images_for_admin()
        self.send_response(200)
        self.send_header('Content-type', 'application/json')
        self.end_headers()
        self.wfile.write(json.dumps({'items': items}).encode('utf-8'))
    ...
    def handle_delete_gallery_image(self):
        """Удаление пользовательского фото главным админом."""
        session = get_current_admin(self.headers.get('Cookie', ''))
        if not session or not session.get('is_super_admin'):
            self.send_error(403)
            return
        ...
        try:
            ...
            success, message = delete_custom_gallery_image(image_id)
            self.send_response(200 if success else 400)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({'success': success, 'message': message}).encode())
        ...
        <body>
            <div class="container">
                ...
                <div id="Media" class="tabcontent">
                    ...
                    <div id="customGalleryWrap" class="control-panel" style="display:{'block' if is_super_admin else 'none'};">
                        <h3>Пользовательские фото на главной</h3>
                        <p style="color:#bbb">Можно удалять только добавленные фото. Стартовые фото клана не затрагиваются.</p>
                        <div id="customGalleryList"></div>
                    </div>
                </div>
            ...
            <script>
                ...
                function loadApplications() {{
                    fetch('/admin/api/applications').then(r => r.json()).then(data => {{
                        ...
                        let html = '<table class="applications-table"><tr><th>ID</th><th>Ник</th><th>Steam</th><th>Часы</th><th>Discord</th><th>Роль</th><th>IP</th><th>Регион</th><th>Сообщение</th><th>Дата</th></tr>';
                        data.applications.forEach(app => {{
                            html += `<tr><td>${{app.id}}</td><td>${{app.nickname}}</td><td>${{app.steam_id}}</td><td>${{app.playtime}}</td><td>${{app.discord}}</td><td>${{app.role}}</td><td>${{app.ip_address || '-'}}</td><td>${{app.region || 'Регион не определён'}}</td><td class="message-cell">${{app.message}}</td><td>${{app.timestamp}}</td></tr>`;
                        }});
                        html += '</table>';
                        list.innerHTML = html;
                    }});
                ...
                function loadPendingGallery() {{
                    ...
                    const customList = document.getElementById('customGalleryList');
                    if (customList) loadCustomGallery();
                }}

                function loadCustomGallery() {{
                    const list = document.getElementById('customGalleryList');
                    if (!list) return;
                    fetch('/admin/api/gallery/custom').then(r => r.json()).then(data => {{
                        if (!data.items.length) {{ list.innerHTML = '<p>Нет пользовательских фото.</p>'; return; }}
                        list.innerHTML = data.items.map(item => `
                            <div class="block-item">
                                <div class="block-header"><span class="block-ip">ID: ${{item.id}}</span><button class="btn btn-danger" onclick="deletePhoto(${{item.id}})">Удалить</button></div>
                                <div class="block-reason">Добавил: ${{item.added_by || 'unknown'}} | ${{item.added_at || ''}}</div>
                                <img class="preview-image" src="${{item.image_url}}" alt="preview">
                                <div><a href="${{item.image_url}}" target="_blank" style="color:#ff9900">${{item.image_url}}</a></div>
                            </div>
                        `).join('');
                ...
                function deletePhoto(imageId) {{
                    if (!confirm(`Удалить фото ID=${{imageId}}?`)) return;
                    fetch('/admin/api/gallery/delete', {{ method: 'POST', headers: {{ 'Content-Type': 'application/json' }}, body: JSON.stringify({{ image_id: imageId }}) }})
                    .then(r => r.json()).then(data => {{
                        alert(data.message);
                        if (data.success) loadCustomGallery();
                    }});
                }}
                ...
                document.getElementById('addImageForm').addEventListener('submit', function(e) {{
                    ...
                    .then(r => r.json()).then(data => {{
                        alert(data.message);
                        if (data.success) {{ this.reset(); loadCustomGallery(); }}
                    }});
                }});
